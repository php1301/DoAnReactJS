...
class MoviesBooking extends Component {
    componentDidMount() {
        ...
        return this.props.items && this.props.items.map((item, index) => {
            return (
                <div className="main-content-container-item">
                    <Link to={`/details/moviebooking/${item.maPhim}`}><img src={item.hinhAnh} style={{ width: "127px", height: "190px" }} /> </Link>
                    <h3 className="main-content-container-item__item-title">{item.tenPhim}</h3>
                    <p className="main-content-container-item__genres">Comedy</p>
                    <p className="main-content-container-item__rating">{star} {item.danhGia} </p>
                </div>
            )
        })
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(MoviesBooking)
